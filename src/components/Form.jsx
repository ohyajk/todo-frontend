...
const poster = {
    ...
    return (
        <form onSubmit={(e) => {e.preventDefault(); addTodo(poster);  e.target.reset(); }} className='my-2 flex flex-col gap-2 md:flex-row md:gap-0 md:bg-[#3b3b3b] md:rounded-full'>
            <input onChange={(e)=> setInput(e.target.value)} className='md:w-[300px] py-2 px-4 outline-none rounded-full md:rounded-l-full text-black md:text-white' placeholder='Add Task Here' type="text" required />
